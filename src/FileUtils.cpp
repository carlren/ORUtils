...
void SaveImageToFileCV(const ORUtils::Image<ORUtils::Vector4<unsigned char> > * image, const char* fileName, bool flipVertical)
{
    cv::Mat mat(image->noDims[1], image->noDims[0], CV_8UC4,
                (void*)image->GetData(MEMORYDEVICE_CPU)->getValues());;
//    cv::imshow("color ", mat);
//    cvWaitKey();
...
{
    cv::Mat mat(image->noDims[1], image->noDims[0], CV_16UC1,
                (void*)image->GetData(MEMORYDEVICE_CPU));;
//    cv::imshow("depth", mat);
//    cvWaitKey();
    cv::imwrite(fileName, mat);
    printf("file saved to : %s\n", fileName);
...
{
    cv::Mat mat(image->noDims[1], image->noDims[0], CV_32FC1,
                (void*)image->GetData(MEMORYDEVICE_CPU));;
//    cv::imshow("depth ", mat);
//    cvWaitKey();
    cv::imwrite(fileName, mat);
...
bool ReadImageFromFile(ORUtils::Image<ORUtils::Vector4<unsigned char> > * image, const char* fileName)
{
    ...
    fileType = fileType.substr(fileType.find_last_of(".")+1, fileType.length());
